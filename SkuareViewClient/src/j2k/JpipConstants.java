...
/**
*
* The class <code>JpipConstants</code> is used to define several
* JPIP global constants.
*
...
public class JpipConstants
{
	...
	*/
	...
	/**
		EOR code sent when the server has transferred all available 
		image information (not just information relevant to the 
		requested view-window) to the client.
	*/
	...
	/**
		EOR code sent when the server has transferred all available 
		information that is relevant to the requested view-window.
	*/
	public static final int EOR_WINDOW_DONE										= 2;
	
	/**
		EOR code sent when the server is terminating its response in 
		order to service a new request.
	*/
	public static final int EOR_WINDOW_CHANGE									= 3;
	
	
	/**
		EOR code sent when the server is terminating its response because 
		the byte limit specified in a byte limit specified in a max length 
		request field has been reached.
	*/
	...
	/**
		EOR code sent when the server is terminating its response because 
		the quality limit specified in a quality request field has been 
		reached.
	*/
	...
	/**
		EOR code sent when the server is terminating its response because
		some limit on the session resources, e.g. a time limit, has been 
		reached. No further request should be issued using a channel ID 
	...
	*/
	...
	/**
		EOR code sent when the server is terminating its response because 
		some limit, e.g., a time limit, has been reached. If the request 
		is issued in a session, further requests can still be issued using 
		a channel ID assigned in that session.
	*/
